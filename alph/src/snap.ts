  ...
  for (let i = 0; i < liquidityTicks.length; i++) {
    ...
    for (let j = visitedTicks.length - 1; j >= 0; j--) {
      ...
      if (liquidityUpper >= liquidityLower) {
        ...
        curr.liquidityChange = (liquidityUpper - liquidityLower) as Liquidity;
        visitedTicks.pop();
      } else {
        liquidityDelta = liquidityUpper;
        prev.liquidityChange = (liquidityLower - liquidityUpper) as Liquidity;
        break;
      }
...
export const createSnapshotForNetwork = async (network: Network) => {
  ...
  switch (network) {
    ...
    case Network.Testnet:
      fileName = "../data/alph/testnet.json";
      snaps = TESTNET_DATA;
      invariantAddress = INVARIANT_ADDRESS.Testnet;
      break;

    case Network.Mainnet:
      fileName = "../data/alph/mainnet.json";
      snaps = MAINNET_DATA;
  ...
  const invariant = await Invariant.load(invariantAddress);
